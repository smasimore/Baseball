...
for ($id = 1; $id < 2500; $id += 30) {
	...
	$target = "http://espn.go.com/mlb/stats/batting/_/count/".$id."/qualified/false";
	$target_expanded = "http://espn.go.com/mlb/stats/batting/_/count/".$id."/qualified/false/type/expanded";
	$target_saber = "http://espn.go.com/mlb/stats/batting/_/count/".$id."/qualified/false/type/sabermetric";
	...
	for ($i=0; $i<count($rowheads); $i++) {
		...
		for ($k = $numcols_reg; $k < $numcols_reg + 15; $k++) {
...
foreach ($player_stats as $stats) {
	...
	insert($database, 'batting_total_2013', $data);
}
