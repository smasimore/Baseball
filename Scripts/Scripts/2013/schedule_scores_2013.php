...
function pullSchedules(array $data, array $teams, array $lineups, array $odds) {
    ...
    foreach ($teams as $id => $team) {
	    ...
	    for ($j = 0; $j < 3; $j++) {
		    ...
		    //first half
		    case 0:
		        $target = 'http://espn.go.com/mlb/team/schedule/_/name/' . $id . '/year/2013/seasontype/2/half/1/' . $team;
		        break;
    		    //second half
	    	    case 1:
                        $target = 'http://espn.go.com/mlb/team/schedule/_/name/' . $id . '/year/2013/seasontype/2/' . $team;
		        break;
    		    //post season
	    	    case 2:
                        $target = 'http://espn.go.com/mlb/team/schedule/_/name/' . $id . '/year/2013/' . $team;
		        break;
    	    }
...
function pullOdds($team_mapping) {
   ...
   for ($month = 3; $month <= 10; $month++) {
       ...
       for ($day = 1; $day <= 31; $day++) {
            ...
            $target = "http://contests.sportsbettingstats.com/Odds.aspx?date=".$month."/".$day."/2013&sport=MLB";
            $source_code = scrape($target);
...
foreach ($schedules as $stats) {
    ...
    insert($database, 'schedule_scores_2013', $data);
}
