...
foreach ($all_players as $player) {
	...
	//Make sure they fielded in 2013 (i.e. DH)
	$fielder_2013 = strpos($source_code, "<td>2013</td><td>");
	if ($fielder_2013 == false) {
		continue;
	}

	$start_2013 = "<td>2013</td><td>";
	$end_2013 = "sponsored";
	
	$source_code_2013 = return_between($source_code, $start_2013, $end_2013, INCL);

	$stats_start = "<td class=\"textright\">";
	$stats_end = "</td>";
	$stats = parse_array_clean($source_code_2013, $stats_start, $stats_end);
	...
	$positions_end = "</td>";
	$positions = parse_array_clean($source_code_2013, $positions_start, $positions_end);

	$team_start = "href=\"[\s\S]+\">";
	$team_end = "</a></li></ul>";
	$teams = parse_array_clean($source_code_2013, $team_start, $team_end);
	...
	$total_positions_start = "<td>2013</td><td>";
	$total_positions_end = "</td><td class=\"textright\">";
	$total_positions = parse_array_clean($source_code_2013, $total_positions_start, $total_positions_end);
	$real_position = array();
	...
	for ($i=0; $i<count($real_position); $i++) {
		...
		$row = array($id, $name, 2013, $team_abbr, $positions[$t]);
...
foreach ($player_array as $stats) {
	...
	insert($database, 'fielding_2013', $data);
}
