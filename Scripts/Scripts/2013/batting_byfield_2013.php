...
/*
$stadium_obp = array();
foreach ($stadium_final as $stadium_name => $stats) {
	$stadium_obp[$stadium_name] = $stats['pct_single'] + $stats['pct_double'] + $stats['pct_triple'] + $stats['pct_home_run'] + $stats['pct_walk'];
}

$stadium_delta = array();
$stadium_delta['colheads'] = array(
	'multiplier',
	'stadium'
);
foreach ($stadiums as $stadium_name => $stadium) {
	$stadium_delta[$stadium_name]['multiplier'] = $stadium_obp[$stadium] / $stadium_obp['Away'];
	$stadium_delta[$stadium_name]['stadium'] = $stadium_name;
}
*/
...
$sql_colheads = $stadium_delta['colheads'];
foreach ($stadium_delta as $stats) {
	if ($stats[0] == 'pct_single') {
		continue;
	} 
	$data = array();
	...
	insert($database, 'batting_byfield_magic_2013', $data);
}
