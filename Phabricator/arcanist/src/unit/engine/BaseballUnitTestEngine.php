...
/**
 * PHPUnit wrapper. Modified from 
 * https://secure.phabricator.com/diffusion/ARC/browse/master/src/unit/engine/PhpunitTestEngine.php.
 */
final class BaseballUnitTestEngine extends ArcanistUnitTestEngine {
  ...
  public function run() {
    ...
    foreach ($this->getPaths() as $path) {
        ...
        if (substr($path, -3) === '.py') {
            if ($test = $this->findTestPythonFile($path)) {
        ...
        if ($test = $this->findTestPHPFile($path)) {
            $this->affectedTests[$path] = $test;
        }
    }
    ...
    foreach ($this->affectedTests as $class_path => $test_path) {
        ...
        if (substr($test_path, -3) == '.py') {
            echo "Testing $test_path \n";
            shell_exec("python $test_path");
            continue;
        }
    ...
    private function findTestPHPFile($path) {
    ...
    private function findTestPythonFile($path) {
        if (substr($path, -7) === 'Test.py') {
            return $path;
        }
        $root = $this->projectRoot;
        $path = Filesystem::resolvePath($path, $root);
